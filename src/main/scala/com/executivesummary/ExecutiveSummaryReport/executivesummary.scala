...
//      println(x); 
  ...
  def main(args : Array[String]): Unit = {
    ...
    val transposedf = transposeDF()
   
    val usagechargedf = getUsageCharge()

    val nonusagedf = getNonUsage()
    ...
    usagechargedf.repartition(1).write.mode("append").format("com.databricks.spark.csv").option("header", "true")
    .save("file:///D:/data/executivesummary")
